...
/**
 * NOTE:
 * - Keeps Tailwind classes as in your original.
 * - Adds more defensive checks when reading project fields from localStorage.
 * - Normalizes tech names (case-insensitive) when picking icons.
 * - Uses a plain <style> block (not style jsx) to avoid CRA/React errors.
 */

/* icon map (we normalize keys to lowercase) */
const TECH_ICONS = {
  react: Globe,
  tailwind: Layout,
  express: Cpu,
...
const getIconForTech = (tech) => {
  if (!tech || typeof tech !== "string") return TECH_ICONS.default;
...
const ProjectStats = ({ project }) => {
  const techStackCount = Array.isArray(project?.TechStack)
    ...
    : 0;
  const featuresCount = Array.isArray(project?.Features)
...
const handleGithubClick = (githubLink) => {
  if (!githubLink || githubLink === "Private" || githubLink === "private") {
    Swal.fire({
      icon: "info",
...
const safeParseProject = (raw) => {
  // raw may be object already or a JSON string — handle gracefully
  if (!raw) return null;
...
const ensureArrayField = (value) => {
  ...
  if (typeof value === "string") {
    // split on commas or newlines; filter empties
    return value
      .split(/[,|\n]/)
      .map((v) => v.trim())
      .filter(Boolean);
...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    // keep same behaviour: scroll to top on mount / id change
    ...
    let storedProjects = [];
    try {
      storedProjects = JSON.parse(rawProjects);
    } catch {
      // if parse fails, try to handle legacy formats
      storedProjects = [];
    }

    if (!Array.isArray(storedProjects)) {
      // try to coerce single-object storage
    ...
    const selectedProject = storedProjects.find((p) => String(p?.id) === String(id));

    if (selectedProject) {
      // make sure Features & TechStack are arrays
      const enhancedProject = {
        ...selectedProject,
        Features: ensureArrayField(selectedProject.Features),
        TechStack: ensureArrayField(selectedProject.TechStack),
        Github:
          selectedProject.Github && selectedProject.Github !== "" ? selectedProject.Github : "Private",
        Link:
          selectedProject.Link && selectedProject.Link !== "" ? selectedProject.Link : "#",
        Img:
          selectedProject.Img && selectedProject.Img !== ""
            ? selectedProject.Img
  ...
  return (
    <div className="min-h-screen bg-[#030014] px-[2%] sm:px-0 relative overflow-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-16">
            <div className="space-y-6 md:space-y-10 animate-slideInLeft">
              ...
              <div className="flex flex-wrap gap-3 md:gap-4">
                ...
                <a
                  href={project.Github && project.Github !== "Private" ? project.Github : "#"}
                  target={project.Github && project.Github !== "Private" ? "_blank" : undefined}
                  rel={project.Github && project.Github !== "Private" ? "noopener noreferrer" : undefined}
      ...
      {/* Plain style block (not styled-jsx) so CRA projects don't break */}
      <style>{`
        @keyframes blob {
